...
s_sub            = _style('Sub',          fontSize=12.5, leading=17, spaceAfter=5,
                          spaceBefore=10, textColor=ACCENT_DARK)
...
def section_banner(num, text):
    """Colored banner header for a section."""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def _draw_footer(self, total):
        ...
        self.drawRightString(PAGE_W - 15 * mm, 11 * mm, '版本 2.0  |  2025')
...
def build_story():
    ...
    # ── 目录 ─────────────────────────────────────────────────────────────────
    story.extend(section_banner('', '目  录'))
    story.append(SP(4))
    toc = [
        ['一',  '病史记录',                                   '主观'],
        ['二',  'NPRS 数字疼痛评定量表',                     '主观'],
        ['三',  'EQ-5D-5L 欧洲五维健康量表',                  '主观'],
        ['四',  '握力测试（Handgrip Strength）',              '客观'],
        ['五',  'SPPB 简易体能状况量表',                      '客观'],
        ['六',  'TUG 起立行走计时测试',                       '客观'],
        ['七',  '2MST 2分钟原地踏步测试（居家适用）',        '客观'],
        ['八-A','ODI Oswestry 功能障碍指数（腰痛专用）',      '专项'],
        ['八-B','QuickDASH 上肢功能评估',                     '专项'],
        ['八-C','ABC Scale 活动特异性平衡信心量表',           '专项'],
        ['八-D','30秒坐站测试',                               '专项'],
        ['八-E','FRT 功能性前伸测试',                         '专项'],
        ['八-F','BBS Berg 平衡量表',                          '专项'],
        ['九',  '评估总结',                                   '汇总'],
        ['附录','MCID 速查表 / Borg RPE / 运动处方 / 评估流程','参考'],
    ]
    rows = [[P('<b>章节</b>', s_th), P('<b>内容</b>', s_th), P('<b>类型</b>', s_th)]]
    for c in toc:
        rows.append([P(c[0], s_td_c), P(c[1], s_td), P(c[2], s_td_c)])
    story.append(make_table(rows, col_widths=cols(1.2, 10, 1.5)))
    story.append(PageBreak())
    ...
    story.append(SP(8))
    story.extend(sub_header('终止标准（安全第一）'))
    for c in ['• 胸痛、胸闷、心悸', '• 严重呼吸困难', '• 头晕、眩晕、共济失调',
              '• 大汗、面色苍白', '• 腿部剧烈疼痛或痉挛',
              P('• SpO<sub>2</sub> < 85%', s_body), '• 患者要求停止']:
        story.append(P(c) if isinstance(c, str) else c)
    story.append(SP(6))
    ...
    # ── 八-D、30秒坐站 ───────────────────────────────────────────────────────
    story.extend(section_banner('八-D', '30秒坐站测试（30-Second Chair Stand Test）'))
    story.append(P('<b>目的：</b>评估下肢力量和耐力。'))
    story.append(P('<b>器材：</b>标准无扶手座椅（座高 43–45 cm），靠墙放置，秒表。'))
    story.append(P('<b>起始姿势：</b>患者坐于椅子中央，双脚分开与肩同宽平踏地面，双手交叉抱于胸前。'))
    story.append(SP(6))
    story.extend(sub_header('操作指令'))
    story.append(P('「当我说"开始"后，请您反复站立再坐下，在 30 秒内尽可能多地完成。每次站起时膝盖必须完全伸直，坐下时臀部需碰到椅面。先做 1–2 次示范，准备好了吗……开始！」'))
    story.append(SP(6))
    test30 = [
        ['重复次数', '________ 次 / 30 秒'],
        ['是否使用双手？', '□ 否  □ 是（如使用双手辅助，需注明）'],
        ['备注', ''],
    ]
    story.append(make_table(test30, col_widths=cols(1, 3), header_rows=0, zebra=False))
    story.append(SP(8))
    story.extend(sub_header('年龄与性别参考值（60–69 岁）'))
    ref30 = [
        [P('<b>百分位</b>', s_th), P('<b>男性（次）</b>', s_th), P('<b>女性（次）</b>', s_th)],
        ['95th（优秀）', '≥ 21', '≥ 19'],
        ['75th（良好）', '16–20', '14–18'],
        ['50th（平均）', '12–15', '10–13'],
        ['25th（低于平均）', '8–11', '7–9'],
    ...
    story.append(make_table(ref30, col_widths=cols(1, 1, 1)))
    story.append(SP(6))
    story.append(P('• <b>低于平均或更差</b>：提示下肢肌力下降，跌倒风险增高。'))
    story.append(P('• <b>MCID：</b>增加 ≥ 2–3 次视为有临床意义的改善。'))
    story.append(PageBreak())

    # ── 八-E、FRT ────────────────────────────────────────────────────────────
    story.extend(section_banner('八-E', '功能性前伸测试（Functional Reach Test, FRT）'))
    story.append(P('<b>目的：</b>评估站立位动态平衡能力/稳定性极限。'))
    story.append(P('<b>器材：</b>墙壁安装的硬质标尺或卷尺，置于肩峰高度。'))
    story.append(SP(6))
    story.extend(sub_header('操作步骤'))
    for s in [
        '1. 患者脱下鞋子，侧对墙壁站立（通常右侧靠近墙壁）。',
        '2. 双脚分开与肩同宽，自然站立。',
        '3. 右臂前屈至 90°（与地面平行），手握拳但不要触碰墙壁。',
        '4. 标尺置于拳面水平，记录此时第三掌骨远端在标尺上的位置（起始位置）。',
        '5. 指令："在不迈步、不失去平衡的前提下，尽可能将手臂和身体前倾。"',
        '6. 记录第三掌骨远端在标尺上到达的最远位置（结束位置）。',
        '7. 前伸距离 = 结束位置 − 起始位置（cm）。',
        '8. 完成 3 次，取最佳值。',
        '<b>注意：脚跟不能离地，脚不能移动，不能旋转躯干接触墙壁。</b>',
    ]:
        story.append(P(s))
    story.append(SP(6))
    frt = [
        [P('', s_th), P('<b>起始位置 (cm)</b>', s_th), P('<b>结束位置 (cm)</b>', s_th), P('<b>前伸距离 (cm)</b>', s_th)],
        ['第 1 次', '', '', ''],
        ['第 2 次', '', '', ''],
        ['第 3 次', '', '', ''],
        [P('<b>最佳成绩</b>', s_td), '—', '—', ''],
    ]
    story.append(make_table(frt, col_widths=cols(2, 4, 4, 4)))
    story.append(SP(8))
    story.extend(sub_header('解读'))
    story.append(P('• <b>≤ 18 cm</b>：跌倒高风险　• <b>18–25 cm</b>：中等风险　• <b>> 25 cm</b>：低风险'))
    story.append(P('• <b>MCID：</b>增加 ≥ 3–5 cm 视为有临床意义的改善。'))
    story.append(SP(4))
    story.append(note('注意：测试时必须有人在患者身边保护，防止跌倒。'))
    story.append(PageBreak())

    # ── 八-F、Berg 平衡量表（NEW） ────────────────────────────────────────────
    story.extend(section_banner('八-F', 'Berg 平衡量表（BBS）'))
    ...
    story.append(SP(4))
    story.append(note('提示：完成 BBS 约需 15–20 分钟。若时间有限，可优先使用 SPPB 平衡子测试 + FRT + TUG 作快速筛查。'))
    story.append(PageBreak())
    ...
    risk_crit = [
        ...
        [P('□ 步速 < 1.0 m/s', s_field), P('□ FRT ≤ 18 cm', s_field)],
        [P('□ ABC < 67%', s_field), P('□ BBS < 45 分', s_field)],
        [P('□ 过去 12 个月跌倒 ≥ 2 次', s_field), P('□ 日常使用助行器/拐杖', s_field)],
    ]
    ...
    mcid = [
        ...
        ['30秒坐站', '次数', '↑ 2–3 次', '低于年龄/性别第25百分位'],
        ['FRT（功能性前伸）', 'cm', '↑ 3–5 cm', '≤ 18 cm 高风险'],
    ]
    ...
    flow = [
        ...
        ['2', '主观评估：NPRS + EQ-5D-5L + GDS-15', '8 min'],
        ...
        ['9', '选做专项测试（ODI / QuickDASH / ABC / 30s坐站 / FRT）', '5–10 min'],
...
def main():
    ...
    parser.add_argument('-o', '--output',
                        default='/Users/zonghanyang/AI Agents/老年物理治疗评估工具手册.pdf',
                        help='输出 PDF 路径')
